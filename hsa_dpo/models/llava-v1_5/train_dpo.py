import os
os.environ["WANDB_PROJECT"]="hsa-dpo"
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, 
        data_path: str,  # HSA-DPO preference data path
        ...
        # Load HSA-DPO preference data from JSONL file
        with open(data_path, "r") as f:
            hsa_dpo_data = [json.loads(line) for line in f]
        
        random.seed(seed)
        
        # Process HSA-DPO data
        list_data_dict = self.hsa_dpo_process(hsa_dpo_data)
        
        rank0_print(f"Loaded {len(list_data_dict)} training samples from HSA-DPO dataset")
        # print(silkie_data_dict)
        # import pdb;
        # pdb.set_trace()
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        chosen_input_ids, chosen_labels, reject_input_ids, reject_labels, chosen_scores, rejected_scores = tuple([instance[key] for instance in instances]
            for key in ("chosen_input_ids", "chosen_labels", "reject_input_ids", "reject_labels", "chosen_scores", "rejected_scores"))
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySupervisedDataset(
        data_path=data_args.desc_data_path,  # Use desc_data_path for HSA-DPO data
        tokenizer=tokenizer,
        data_args=data_args)
...
def main():
    ...
    if script_args.lora_enable:
        state_dict = get_peft_state_maybe_zero_3(
            llava_policy_model.named_parameters(), training_args.lora_bias
        )
        non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(
            llava_policy_model.named_parameters()
        )
        if script_args.local_rank == 0 or script_args.local_rank == -1:
        ...
        safe_save_model_for_hf_trainer(trainer=dpo_trainer,
