...
\begin{abstract}
Large vision-language models (LVLMs) can generate fluent answers that contain visually unsupported objects, attributes, or relations. This hallucination problem is difficult to mitigate with coarse response-level labels because a single response may contain both grounded and ungrounded statements with different severities. In this work, we implement and analyze a fine-grained hallucination mitigation pipeline that converts released hallucination supervision into structured critiques, uses those critiques for rewrite-based preference construction, optionally validates and repairs preference pairs with LVLM judges, and trains LLaVA adapters with DPO-style objectives. We compare a base LLaVA model, standard DPO, direct HSA-DPO, and a judged severity-margin HSA-DPO variant. The results are mixed but informative: POPE adversarial F1 remains slightly higher for the base model ($84.18$) than for the trained variants ($83.87$--$83.91$), while Object HalBench and AMBER hallucination metrics improve substantially after alignment. In particular, Object HalBench CHAIRs decreases from $53.00$ to $36.00$ for the judged severity-margin variant, and AMBER-Hal decreases from $35.90$ to approximately $25$. These findings suggest that fine-grained critique and severity-aware preference training are useful for reducing generative hallucination, but their benefits are benchmark-dependent and do not automatically transfer to every hallucination evaluation setting.
\end{abstract}
...
Our methodology treats hallucination mitigation as a data construction and preference-alignment problem rather than only a model-training problem. The pipeline begins with released fine-grained hallucination supervision, converts it into normalized critique records, constructs or reuses preference pairs, optionally validates and repairs those pairs, and finally trains LVLM adapters with DPO-style objectives. Figure~\ref{fig:pipeline} summarizes the resulting workflow. The design has two goals. First, it preserves the fine-grained supervision signal: hallucination type, rationale, evidence text, and severity are retained instead of being collapsed into a single binary label. Second, it separates two preference-data paths: a direct path that trains on the released preference pairs as provided, and a judged path that uses Gemini/GPT checks plus LLaVA repair to reduce noisy or weak pairs before training.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{../diagram_overview.png}
  \caption{Overview of the implemented fine-grained hallucination mitigation pipeline. Released supervision is normalized into critique records, LLaVA is used for critique-guided rewriting or repair, preference pairs are routed through direct and judged paths, and the resulting training sets are used for DPO/HSA-DPO style severity-aware alignment.}
  \label{fig:pipeline}
\end{figure}
...
The direct path uses the released preference data without additional API judging. This path preserves the original chosen/rejected pairs and severity scores and is used to train a direct HSA-DPO model and a matched normal DPO baseline. The purpose of this path is to establish a clean baseline: if the released preference dataset already contains useful alignment signal, then direct DPO/HSA-DPO training should be a strong and simple reference point.
...
Pairs rejected by the judged path are not automatically discarded. They are routed to a repair stage in which LLaVA receives the original rejected response, the current chosen response, severity information, and the judge feedback. LLaVA then produces a repaired chosen response. The repaired pair is checked for basic validity and merged with accepted pairs to form the judged-preference training set. This design separates diagnosis from repair: Gemini/GPT are used as quality checkers, while LLaVA remains the local model used to produce repaired training text.
...
\begin{equation}
D_B = D_{\mathrm{accepted}} \cup D_{\mathrm{repaired}},
\end{equation}
where $D_{\mathrm{accepted}}$ contains judge-approved pairs and $D_{\mathrm{repaired}}$ contains repaired versions of rejected pairs. These two training sets allow the project to compare whether extra preference quality control helps or whether direct training on released preferences is already sufficient.
...
The implementation is organized to make the full workflow reproducible and inspectable. Each stage writes JSONL artifacts and a statistics file. Stage 1 writes normalized fine-grained critique records. Stage 2 writes detector-style examples. Released-preference Stage 3 writes accepted pairs, rejected pairs, and audit records containing judge decisions. Released-preference Stage 4 writes repaired pairs and the final merged preference dataset. Stage 5 writes LoRA adapters and trainer states for the aligned models.

The current repository provides separate scripts for the direct and judged paths. The direct path trains from the released preference file and supports both HSA-DPO and standard DPO. The judged path first validates released pairs, repairs rejected rows, and then trains on the merged accepted/repaired preference dataset. Evaluation is kept separate from training and is reported through automatic benchmark summaries, with paper-comparable and supplemental local results separated to avoid overstating conclusions.

\section{Experiments}

We evaluated the proposed mitigation pipeline using a controlled comparison built around LLaVA-1.5-7B as the base large vision-language model. The released fine-grained hallucination supervision was first normalized into $16{,}143$ feedback records, consisting of $7{,}643$ hallucinated samples and $8{,}500$ non-hallucinated samples, and the main alignment experiments used $8{,}386$ image-response preference pairs. We compared the unaligned LLaVA-1.5-7B model, a standard DPO model trained on the same preference pairs, a severity-aware HSA-DPO model, and judged-preference variants in which low-quality pairs were either repaired or removed before training. To ensure a fair comparison, the standard DPO and HSA-DPO runs used the same training budget and optimization settings: one training epoch, per-device batch size $1$, effective batch size $32$, learning rate $2\times10^{-6}$, DPO temperature parameter $\beta=0.1$, LoRA rank $128$, LoRA scaling factor $256$, maximum sequence length $1024$, cosine learning-rate scheduling, no warmup, and zero weight decay. The HSA-DPO variant differed only in its objective, where severity scores were used to assign stronger penalties to preference pairs containing more severe hallucinations, while the standard DPO baseline treated all preference pairs uniformly. Model behavior was evaluated using hallucination-oriented benchmarks including POPE, Object HalBench, and AMBER, with the analysis focusing on precision, recall, F1 score, and hallucination-related error rates rather than claiming uniform improvement across all benchmarks.

\section{Results}

Table~\ref{tab:pope-results} reports POPE adversarial results for the base model and the three trained variants. On this binary object-presence benchmark, alignment does not improve the main F1 score. The base LLaVA model obtains the highest F1 score, $84.18$, while direct HSA-DPO reaches $83.91$, standard DPO reaches $83.88$, and the judged severity-margin HSA-DPO variant reaches $83.87$. The trained models increase precision relative to the base model, with direct HSA-DPO reaching $90.70$ precision compared with $89.92$ for the base model. However, this precision gain is offset by lower recall: the base model obtains $79.13$ recall, while the trained variants remain around $78.07$--$78.33$. This pattern indicates that preference training made the models somewhat more conservative on object-presence decisions, but the reduction in recall prevented an overall F1 improvement.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    Direct normal DPO & 85.00 & 90.63 & 78.07 & 83.88 \\
    Direct HSA-DPO & 85.03 & 90.70 & 78.07 & 83.91 \\
    Judged margin HSA-DPO & 84.93 & 90.25 & 78.33 & 83.87 \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:hallucination-results} reports Object HalBench and AMBER hallucination-oriented metrics. These results show a clearer benefit from the mitigation pipeline. On Object HalBench, all trained variants reduce hallucination rates substantially. CHAIRs decreases from $53.00$ for the base model to $38.00$ for direct HSA-DPO, $37.67$ for standard DPO, and $36.00$ for judged severity-margin HSA-DPO. CHAIRi follows the same trend, decreasing from $15.72$ to $11.74$, $11.98$, and $10.41$, respectively. The judged severity-margin variant therefore gives the strongest Object HalBench result among the completed runs.

AMBER shows a similar improvement on hallucination-related metrics. AMBER-CHAIR decreases from $7.70$ for the base model to $5.50$ for direct HSA-DPO, $5.20$ for standard DPO, and $5.30$ for judged severity-margin HSA-DPO. AMBER-Hal decreases from $35.90$ to $25.10$, $25.20$, and $25.50$, respectively. AMBER-Cog also decreases from $4.30$ to $2.50$ for all trained variants. The only tradeoff is AMBER-Cover, which decreases from $51.70$ for the base model to approximately $49.60$--$50.00$ after training. This suggests that the aligned models reduce unsupported content, but may also become slightly more conservative in the amount of visual detail they include.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    Direct normal DPO & 37.67 & 11.98 & 5.20 & 25.20 & 2.50 & 49.90 \\
    Direct HSA-DPO & 38.00 & 11.74 & 5.50 & 25.10 & 2.50 & 50.00 \\
    Judged margin HSA-DPO & 36.00 & 10.41 & 5.30 & 25.50 & 2.50 & 49.60 \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.74\linewidth,height=0.18\textheight]{../result_figures/pope_benchmark_histogram.png}\\[0.25em]
  \includegraphics[width=0.74\linewidth,height=0.18\textheight]{../result_figures/object_halbench_histogram.png}\\[0.25em]
  \includegraphics[width=0.74\linewidth,height=0.18\textheight]{../result_figures/amber_benchmark_histogram.png}
  \caption{Histogram summaries for POPE, Object HalBench, and AMBER. The plots visualize the same benchmark patterns as Tables~\ref{tab:pope-results} and~\ref{tab:hallucination-results}: limited POPE improvement, stronger reductions in Object HalBench hallucination rates, and lower AMBER hallucination-related metrics after mitigation.}
...
\begin{ack}
This section is intentionally left blank for the anonymized submission. Funding and acknowledgment details can be added in the camera-ready version if the paper is accepted.
\end{ack}
...
When a preference pair fails validation, LLaVA can repair the chosen response. The repair prompt uses the original response, the failed rewrite, the fine-grained critiques, and the validation reasons from the judges.
...
\textbf{Previous rewrite that failed validation:}
\texttt{\{failed rewrite\}}

\textbf{Stage 1 critiques:}
\texttt{\{critique list\}}

\textbf{Stage 3 validation feedback:}
\texttt{\{judge votes and reasons\}}

\textbf{Instructions.}
Produce a corrected final answer that should be preferred over the original response. Fix every hallucination identified in the critiques. Address the validation reasons, especially rejected votes. Preserve correct supported visual content from the original response. Do not introduce new unsupported objects, attributes, relations, counts, actions, or identities. Output only the repaired answer. Do not include markdown, labels, or explanations.
\end{quote}

\subsection{Repaired-Pair Verification Prompt}
\label{app:repair-verify-prompt}

After LLaVA repair, the same Gemini/GPT validation prompt in Appendix~\ref{app:api-validation-prompt} is reused to verify repaired pairs. The repaired answer is placed in the chosen-response field, the original hallucinated answer remains the rejected response, and the judge returns the same JSON schema:
\begin{quote}
\small
\texttt{\{"approved": true/false, "reason": "one concise sentence"\}}
\end{quote}
Approved repaired rows are merged back into the judged-preference training set. Failed repaired rows are removed from that judged path.
