...
from fg_pipeline.eval.judges import judge_mmhal_rows
...
class MMHalBenchmark:
    ...
    def build_spec(
        ...
        *,
        ...
        dataset_root = default_dataset_root(dataset_root_override, "mmhal-bench")
    ...
    def evaluate(
        ...
        *,
        ...
        if model is None:
            raise ValueError("MMHal-Bench requires a model spec")
        if not openai_judge_model:
            raise RuntimeError("MMHal-Bench requires --openai-judge-model")
        question_path = Path(spec.question_file or "")
        if not question_path.exists():
            raise FileNotFoundError(f"Missing MMHal-Bench question file: {question_path}")
        ...
        with prediction_path.open("w", encoding="utf-8", newline="\n") as handle:
            for row in answers:
                ...
                handle.write("\n")
        judged_rows, summary = judge_mmhal_rows(answers, openai_judge_model)
        ...
        metric_artifact = MetricArtifact(
            ...
            metrics={
                "mmhal_overall": summary["overall"],
                "mmhal_resp": summary["response_hallucination_rate"],
            },
            comparable_to_paper=False,
            comparison_note="project-owned JSON judge, not the official MMHal evaluator",
        )
        ...
        dump_json(metric_dir / f"{self.name}.meta.json", {"question_file": spec.question_file, "num_examples": len(answers)})
        return (
            PredictionArtifact(
                benchmark=self.name,
                model_id=model.model_id,
